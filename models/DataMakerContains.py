    ...
    while attempts < MAX_RETRIES:
        ...
        try:
            with connection.cursor() as cursor:
                insert_query = "INSERT INTO CONTAINS (USERNAME, COLLECTIONID, VIDEOGAMEID) VALUES (%s, %s, %s)"

                # inserting the items individually bc I could not get a bulk insert to work
                for item in lst:
                    cursor.execute(insert_query, (item[0].strip(","), item[1].strip(","), item[2].strip(",")))
...
def main():
    ...
    with open('contains') as file:
        for line in file:
            lst.append(tuple(line.strip().split(" ")))

    insert_data(lst, MAX_RETRIES)
