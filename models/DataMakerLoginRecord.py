...
def insert_data(follow_lst, MAX_RETRIES):
    attempts = 0
    ...
    while attempts < MAX_RETRIES:
        ...
        try:
            with connection.cursor() as cursor:
                insert_query = "INSERT INTO LOGIN_RECORD (USERNAME, LOGIN_DATE) VALUES (%s, %s)"
...
def main():
    ...
    with open('login_record') as file:
        for line in file:
            print(line)
            lst = line.strip().split("|")
            item0 = lst[0][0:-1]
            item1 = lst[1][1:-1]
            print(tuple([item0,item1]))
            follow_lst.append(tuple([item0,item1]))
