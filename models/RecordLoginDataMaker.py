...
def insert_record_login_data(follow_lst, MAX_RETRIES):
    attempts = 0
...
def main():
    ...
    # The pairs in follows come from ChatGPT making up users who logged in
    with open('logins') as file:
        for line in file:
            print(line)
            print(tuple(line.strip().split("\t")))
            follow_lst.append(tuple(line.strip().split("\t")))

    insert_record_login_data(follow_lst, MAX_RETRIES)
