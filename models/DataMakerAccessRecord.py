...
def insert_access_record_data(follow_lst, MAX_RETRIES):
    attempts = 0
    ...
    while attempts < MAX_RETRIES:
        ...
        try:
            with connection.cursor() as cursor:
                insert_query = "INSERT INTO ACCESS_RECORDS (ACCESSID, CREATION_DATE) VALUES (%s, %s)"
...
def main():
    ...
    for i in range(1, 6000):
        access_date = fake.date_this_decade().strftime('%Y-%m-%d %H:%M:%S')
        follow_lst.append((i, access_date))

    insert_access_record_data(follow_lst, MAX_RETRIES)
