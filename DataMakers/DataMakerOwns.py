...
# maximum number of connection retries - arbitrarily chosen
MAX_RETRIES = 4
...
def insert_owns_data(follow_lst, MAX_RETRIES):
    attempts = 0
    ...
    while attempts < MAX_RETRIES:
        ...
        try:
            with connection.cursor() as cursor:
                insert_query = "INSERT INTO OWNS (USERNAME, PLATFORMID) VALUES (%s, %s)"
...
def main():
    ...
    # The pairs in follows come from ChatGPT pairing users and platforms
    with open('user_owns_platform') as file:
        for line in file:
            print(line)
            print(tuple(line.strip().split("\t")))
            follow_lst.append(tuple(line.strip().split("\t")))

    insert_owns_data(follow_lst, MAX_RETRIES)
