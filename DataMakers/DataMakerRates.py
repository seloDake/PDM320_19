...
def insert_rates_data(rates_lst, MAX_RETRIES):
    attempts = 0
    ...
    while attempts < MAX_RETRIES:
        ...
        try:
            with connection.cursor() as cursor:
                insert_query = "INSERT INTO RATES (USERNAME, VIDEOGAMEID, RATING) VALUES (%s, %s, %s)"

                # inserting the items individually bc I could not get a bulk insert to work
                for item in rates_lst:
                    cursor.execute(insert_query, (item[0], item[1], item[2]))
...
def main():
    ...
    # The pairs in follows come from ChatGPT pairing users, games, and ratings
    with open('ratings') as file:
        for line in file:
            rates_lst.append(tuple(line.strip().split(" ")))

    insert_rates_data(rates_lst, MAX_RETRIES)
